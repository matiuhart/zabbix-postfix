...
FPOS=/tmp/zabbix-postfix-offset-totals.dat
...
# Crontab
DATA="$(${LOGTAIL} -o ${FPOS} ${MAILLOG} | ${PFLOGSUMM} -u 5 -h 5 --deferral_detail=0 --deferral_detail=0 --reject_detail=0 --no_no_msg_size --smtpd_warning_detail=0 --problems_first -d today /var/log/mail.log)"


zsend pf.totals.day[received] $(echo -e "${DATA}" | grep -m 1 received | cut -f1 -d"r")
zsend pf.totals.day[delivered] $(echo -e "${DATA}" | grep -m 1 delivered | cut -f1 -d"d")
